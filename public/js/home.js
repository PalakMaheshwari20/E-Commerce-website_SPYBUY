const setupSlidingEffect = () => {
    const productContainers = [...document.querySelectorAll('.product-container')]; /*select all product containers */
    const nxtBtn = [...document.querySelectorAll('.nxt-btn')]; /*select all next button */
    const preBtn = [...document.querySelectorAll('.pre-btn')]; /*select all pre button */

    productContainers.forEach((item, i) => {    /*loop through each card */
        ...
        if(data[i].id != decodeURI(location.pathname.split('/').pop())){
            ...
            <div class="product-card">
                ...
                <div class="product-info" onclick="location.href = '/products/${data[i].id}'">
                    ...
                    <span class="price">Rs.${data[i].sellPrice}</span><span class="actual-price">Rs.${data[i].actualPrice}</span>
                </div>
            </div>
            `
        }
